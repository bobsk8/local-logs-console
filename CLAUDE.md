...
- **Webview assets are loaded from `src/webview/` at runtime**, not from `out/`. `tsc` only compiles `.ts`, so `logDashboard.ts` references `src/webview/style.css` and `src/webview/script.js` directly via `asWebviewUri`. This is why `package.json`'s `files` array explicitly ships `src/webview/**` alongside `out/**`. Do not move these files into `out/` or rename the folder without updating both. Packaging uses only the `files` allow-list — `vsce` refuses to build if a `.vscodeignore` is reintroduced alongside it ("Both a .vscodeignore file and a files property... were found"), so don't add one back.
